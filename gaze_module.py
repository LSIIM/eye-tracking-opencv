...
class EyeGazeEstimator():
    def __init__(self, face_mesh, frame_shape):
    ...
    def _relative(self, point):
        ...
        return (int(point.x * self.frame_shape[1]), int(point.y * self.frame_shape[0]))
    ...
    def _relativeT(self, point):
        ...
        return (int(point.x * self.frame_shape[1]), int(point.y * self.frame_shape[0], 0))
    ...
    def get_eye_gaze_vector(self, face_lms):
        ...
        image_points_2D = np.array([
            self._relative(face_lms.landmark[4]),  # Nose tip
            self._relative(face_lms.landmark[152]),  # Chin
            self._relative(face_lms.landmark[263]),  # Left eye left corner
            self._relative(face_lms.landmark[33]),  # Right eye right corner
            self._relative(face_lms.landmark[287]),  # Left Mouth corner
            self._relative(face_lms.landmark[57])  # Right mouth corner
        ], dtype="double")

        # 2D.5 image points (it still 3d, nut the z is 0)
        image_points_2D5 = np.array([
            self._relativeT(face_lms.landmark[4]),  # Nose tip
            self._relativeT(face_lms.landmark[152]),  # Chin
            self._relativeT(face_lms.landmark[263]),  # Left eye, left corner
            self._relativeT(face_lms.landmark[33]),  # Right eye, right corner
            self._relativeT(face_lms.landmark[287]),  # Left Mouth corner
            self._relativeT(face_lms.landmark[57])  # Right mouth corner
        ], dtype="double")
        ...
        # 2D Pupil location
        left_eye = self._relative(face_lms.landmark[468])
        right_eye = self._relative(face_lms.landmark[473])

        # Transformation between image point to world point
        _, transformation = cv2.estimateAffine3D(image_points_2D5, self.model_points)
        ...
        if transformation is not None: # if estimateAffine3D succeeded
            ...
            right_S_vector = self.eye_ball_center_right + (right_pupil_world_cord - self.eye_ball_center_right) * gaze_distance_scale

            # Project a 3D gaze direction into the image plane
            (left_eye_pupil_2d, _) = cv2.projectPoints(left_S_vector, rotation_vector, translation_vector, self.camera_matrix, self.dist_coeffs)
            (right_eye_pupil_2d, _) = cv2.projectPoints(right_S_vector, rotation_vector, translation_vector, self.camera_matrix, self.dist_coeffs)

            # Project 3D head Pose into the image plane
            (head_pose, _) = cv2.projectPoints((int(left_pupil_world_cord[0]), int(left_pupil_world_cord[1]), 40), rotation_vector, translation_vector, self.camera_matrix, self.dist_coeffs)

            # correct gaze vector for the head pose
            left_eye_gaze = left_eye + (left_eye_pupil_2d[0][0] - left_eye) - (head_pose[0][0] - left_eye)
            right_eye_gaze = right_eye + (right_eye_pupil_2d[0][0] - right_eye) - (head_pose[0][0] - right_eye)

            print(f'Left Eye Gaze: {left_eye_gaze}')
            print(f'Right Eye Gaze: {right_eye_gaze}')
